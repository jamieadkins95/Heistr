...
public class ArrayAdapterSkillTierList extends ArrayAdapter<SkillTier>{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        btns[0] = (Button) rowView.findViewById(R.id.btnSkill1);
        btns[1] = (Button) rowView.findViewById(R.id.btnSkill2);
        btns[2] = (Button) rowView.findViewById(R.id.btnSkill3);

        for (int i =0; i < skillTiers.get(position).getSkillsInTier().size(); i++){
            btns[i].setText(skillTiers.get(position).getSkillsInTier().get(i).getName().substring(0, 3));
        }
